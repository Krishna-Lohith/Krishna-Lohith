
# Hi there, I'm Lohith Mothukuri 👋

**AI/ML Engineer** | Python · GenAI · MLOps | 📍 Atlanta, GA (Open to Relocation)

> 🟢 Actively seeking mid-level AI/ML Engineer roles in the USA

---

## 🤖 About Me

```python
profile = {
  "role": "AI/ML Engineer @ Visa | Ex-American Airlines",
  "education": "MS Computer Science — Auburn University (2025)",
  "focus": ["RAG Pipelines", "Transaction Risk Modeling", "MLOps"],
  "looking_for": "Mid-level AI/ML Engineer roles in the USA",
}
```

---

## ⚡ By the Numbers

| 3+ Years Experience | 1M+ Daily Txns Modeled | 35% Policy Lookup Reduction |
|---|---|---|

---

## 🛠 Tech Stack

**Languages:** Python · SQL

**ML / AI:**
![PyTorch](https://img.shields.io/badge/PyTorch-EE4C2C?style=flat&logo=pytorch&logoColor=white)
![XGBoost](https://img.shields.io/badge/XGBoost-189AB4?style=flat)
![Scikit-learn](https://img.shields.io/badge/Scikit--learn-F7931E?style=flat&logo=scikit-learn&logoColor=white)
![HuggingFace](https://img.shields.io/badge/HuggingFace-FFD21E?style=flat&logo=huggingface&logoColor=black)

**GenAI / LLMs:**
![LangChain](https://img.shields.io/badge/LangChain-1C3C3C?style=flat)
![FAISS](https://img.shields.io/badge/FAISS-00599C?style=flat)
![Pinecone](https://img.shields.io/badge/Pinecone-000000?style=flat)
![GPT-4](https://img.shields.io/badge/GPT--4-412991?style=flat&logo=openai&logoColor=white)

**MLOps / Cloud:**
![FastAPI](https://img.shields.io/badge/FastAPI-009688?style=flat&logo=fastapi&logoColor=white)
![AWS](https://img.shields.io/badge/AWS-232F3E?style=flat&logo=amazonaws&logoColor=white)
![MLflow](https://img.shields.io/badge/MLflow-0194E2?style=flat&logo=mlflow&logoColor=white)
![Docker](https://img.shields.io/badge/Docker-2496ED?style=flat&logo=docker&logoColor=white)

---

## 💼 Experience

**🔵 Machine Learning Engineer — Visa** *(Aug 2024 – Present · Atlanta, GA)*
`Transaction Risk` `RAG` `Anomaly Detection` `NLP` `FastAPI`

**🔵 Data Scientist — American Airlines** *(Dec 2022 – Jul 2023 · Hyderabad, India)*
`Delay Prediction` `Power BI` `Airflow` `Redshift`

**🔵 Data Analyst — Concentrix (BFSI)** *(Sep 2021 – Dec 2022 · Hyderabad, India)*
`Oracle SQL` `ETL` `Power BI` `Reporting`

---

## 🚀 Featured Project

### 🏆 Accident Detection & Alert System — IoT
> 1st Prize · 36-Hour Hackathon · JNTU Hyderabad

Real-time accident detection using IoT sensors + Raspberry Pi with GPS-triggered emergency alerts, built end-to-end in Python.

`Python` `Raspberry Pi` `IoT` `GPS` `Alert Automation`

---

## 📬 Connect

[![LinkedIn](https://img.shields.io/badge/LinkedIn-0077B5?style=flat&logo=linkedin&logoColor=white)](https://www.linkedin.com/in/lohithmothu)
[![GitHub](https://img.shields.io/badge/GitHub-100000?style=flat&logo=github&logoColor=white)](https://github.com/krishna-lohith)
[![Email](https://img.shields.io/badge/Email-D14836?style=flat&logo=gmail&logoColor=white)](mailto:lohith.mothu@gmail.com)

✅ **Open to Work** — AI/ML Roles USA
